...
class Simulation
  ...
  def tick(input)
    ...
    when "REPORT"
      puts "#{next_robot_state.x},#{next_robot_state.y},#{next_robot_state.direction.upcase}"
    else
      puts "Invalid command. Please try again."
    end

    @robot = next_robot_state if @grid.valid_position?(next_robot_state.x, next_robot_state.y)
  end
end
